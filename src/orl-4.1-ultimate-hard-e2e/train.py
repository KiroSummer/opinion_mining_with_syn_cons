...
Mdify:
    2020.08.06: support the end-to-end mode
    2020.07.25: hard MTL for cons (cons, and cons gcn)
    2020.07.24: check the four method
    2020-07.24: fix the bert-based cons gcn
    2020.07.23: soft MTL higher level for orl span concat cons span
    2020.07.23: soft MTL, modify the code
...
def evaluate_tagger(model, batched_dev_data, orl_cons, auto_dep_data, dev_gold_spans, label_dict, config,
    ...
    with torch.no_grad():
        ...
        for i, batched_tensor in enumerate(batched_dev_data):
            ...
            cons_trees = None if config.use_cons_gcn is False \
                else [orl_cons[' '.join(words)] for words in sent_words]
            auto_dep_trees = None if config.use_dep_gcn is False \
                else [auto_dep_data[' '.join(words)] for words in sent_words]
    ...
    conll_f1 = compute_orl_fl(dev_gold_spans, dse_predictions, srl_predictions, is_gold_dse=config.use_gold_predicates)
...
def train_tagger(args):
    ...
    with Timer('Preparation'):
        ...
        data.cons_non_terminal_dict.save(os.path.join(args.model, 'cons_non_terminal_dict'))
    ...
    while global_step <= 320000:  # epoch < config.max_epochs
        ...
        with Timer("Epoch%d" % epoch) as timer:
            if epoch == 0 and config.hot_start is True:
                model.use_gold_predicates = True
            else:
                model.use_gold_predicates = False
            print("set model use_gold_predicates to ", model.use_gold_predicates)
            model.train()
            mixed_data = []
            ...
            for batch_num, batch_data in enumerate(batched_data):  # for each batch in the training corpus
                if config.mtl_cons and epoch == 0 and batch_num < 100:  # about 100 batch of ORL data
                    model.use_gold_predicates = True
                else:
                    model.use_gold_predicates = False
                ...
                if config.mtl_cons:  # do the MTL cons
                    ...
                    cons_sent_lengths, seqs_text, cons_word_indexes, cons_char_indexes,\
                    ...
                    auto_cons_trees = None if config.use_cons_gcn is False \
                        else [data.auto_cons_data[' '.join(words)] for words in seqs_text]
                ...
                if config.mtl_dep:
                    # dep forward
                    seqs_text, word_indexes, char_indexes, mask, lengths, heads, labels = batched_dep_tensor
                    if args.gpu:
                    ...
                    _, dep_loss = model.dep_forward(
                        lengths,
                        (seqs_text, word_indexes, char_indexes),
                        None, None, None, None,
                        (heads, labels)
                    )
                ...
                sent_words = batched_tensor
                auto_cons_trees = None if config.use_cons_gcn is False \
                    else [data.auto_cons_data[' '.join(words)] for words in sent_words]
                auto_dep_trees = None if config.use_dep_gcn is False \
                    else [data.auto_dep_data[' '.join(words)] for words in sent_words]
                if args.gpu:
                    word_indexes, char_indexes, \
                    dse_starts, dse_ends, dse_starts_one_hot, dse_ends_one_hot, \
                    arg_starts, arg_ends, arg_starts_one_hot, arg_ends_one_hot, \
                ...
                if i % 250 == 0:
                    ...
                    train_loss = 0.0
                    i = 0
                if global_step % 100 == 0:  # evaluate for every 100 steps
