...
class TaggerData(object):
    def __init__(self, config,
        ...
        self.cons_non_terminal_dict = cons_non_terminal_dict
    ...
    def tensorize_batch_cons_samples(self, samples):
        ...
        max_sample_word_length = max([sam[3].shape[1] for sam in samples])

        max_sample_cons_number = max([len(sam[4][0]) for sam in samples])
        # input
        ...
        for i, sample in enumerate(samples):
            ...
            batch_samples_texts.append(sample[1])
            padded_sample_lengths[i] = sample_length
            # input
            padded_word_tokens[i, :sample_length] = sample[2]
            padded_char_tokens[i, :sample[3].shape[0], : sample[3].shape[1]] = sample[3]
            # gold cons
            sample_cons_number = len(sample[4][0])
            padded_cons_starts[i, :sample_cons_number] = sample[4][0]
            padded_cons_ends[i, :sample_cons_number] = sample[4][1]
            padded_cons_labels[i, :sample_cons_number] = sample[4][2]
            padded_num_gold_cons[i] = sample_cons_number

            padded_gold_cons_starts_one_hot[i][list(sample[4][0])] = 1
            padded_gold_cons_ends_one_hot[i][list(sample[4][1])] = 1
        return torch.from_numpy(padded_sample_lengths), batch_samples_texts, \
    ...
    def tensorize_dep_samples(self, samples):
        ...
        max_sample_length = max([len(sam[1]) for sam in samples])
        max_sample_word_length = max([sam[2].shape[1] for sam in samples])
        # input
        lengths = np.zeros(batch_num_samples, dtype=np.int)
        batch_texts = [sam[0] for sam in samples]
        ...
        for i, sample in enumerate(samples):
            ...
            lengths[i] = length
            # input
            padded_word_tokens[i, :length] = sample[1]
            padded_char_tokens[i, :sample[2].shape[0], :sample[2].shape[1]] = sample[2]
            masks[i, :length] = np.ones((length), dtype=np.int64)
            # output
            heads.append(torch.from_numpy(sample[3]))
            labels.append(torch.from_numpy(sample[4]))
        return batch_texts, torch.from_numpy(padded_word_tokens), torch.from_numpy(padded_char_tokens), torch.from_numpy(masks), \
