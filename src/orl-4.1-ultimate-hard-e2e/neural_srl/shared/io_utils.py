...
def write_predprops_to(predictions,
                       label_dict,
                       input_file,
                       output_file,
                       gold_props_file=None,
                       output_props_file=None):
  ...
  """
    ...
    if output_props_file is not None and output_props_file != '':
        fout_props = open(output_props_file, 'w')
    else:
        fout_props = None

    if gold_props_file is not None and gold_props_file != '':
    ...
    for line in fin:
        ...
        for (t, p) in enumerate(pred):
            if label_dict.idx2str[p] == 'V':
                ...
                out_tags[t] = 'B-V'
                out_line = str(t) + '\t' + raw_sent + ' ||| ' + ' '.join(
                    out_tags) + '\n'
                fout.write(out_line)
                props.append(t)

        if fout_props is not None:
            if sent_id > 0:
                fout_props.write('\n')
            for t in range(slen):
    ...
    fout.close()
    print('Predicted predicates in sequential-tagging format written to: {}.'.
          format(output_file))
    if fout_props is not None:
        fout_props.close()
        print('CoNLL-formatted predicate information written to: {}.'.format(
            output_props_file))
...
def bio_to_spans(predictions, label_dict):
  ...
  """
    ...
    for (i, tag) in enumerate(tags):
        ...
        # Close current span.
        if i == len(predictions) - 1 or tags[
                i + 1][0] == 'B' or label != tags[i + 1][2:]:
            args[-1][2] = i
    return args


def print_to_readable(predictions, num_tokens, label_dict, input_path,
                      output_path):
