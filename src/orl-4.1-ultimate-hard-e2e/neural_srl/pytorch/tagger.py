...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        self.cons_non_terminal_dict_size = data.cons_non_terminal_dict.size()
        self.cuda_id = gpu_id
        # needed constituent spans
        self.needed_cons_labels = ["NP", "VP", "SBAR", "PP", "NML", "ADVP", "ADJP"]
        # self.needed_cons_labels = ["SBAR", "PP", "NML", "ADVP", "ADJP"]
        # self.needed_cons_labels = ["NP", "VP"]
        ...
        self.non_terminal_word_embedding = nn.Embedding(self.cons_non_terminal_dict_size, self.word_embedding_dim, padding_idx=0)
        self.non_terminal_bert_embedding = nn.Embedding(self.cons_non_terminal_dict_size, self.bert_dim, padding_idx=0)
        ...
        self.implicit_dependency_representation = ImplicitDependencyRepresentations(
            config, self.input_size, self.lstm_hidden_size, self.dep_label_space_size
        )
    ...
    def reset_parameters(self):
        ...
        init.xavier_uniform_(self.non_terminal_word_embedding.weight)
        init.xavier_uniform_(self.non_terminal_bert_embedding.weight)
