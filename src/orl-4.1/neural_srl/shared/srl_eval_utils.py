...
def compute_orl_fl(gold_orl, dse_predictions, predictions, gold_dse=True):
        ...
        if mode == "SYS":
            ...
            for sys_dse_arg in spans:
                ...
                if label == "AGENT":
                    ...
                    # holder_metric.add_proportion_gold(sys_a_len)
                else:
                    assert label == "TARGET"
                    target_metic.add_exact_gold(1)
                    target_metic.add_binary_gold(1)
                    # target_metic.add_proportion_gold(sys_a_len)
    ...
    for gold_span, sys_span in zip(gold_spans, predictions):
        ...
        for sys_dse in sys_span:
            ...
            # exact F1 score
            gold_span_num += len(gold_args)
            sys_span_num += len(sys_args)

            add_into_sys_and_gold(sys_args, "SYS")
            add_into_sys_and_gold(gold_args, "GOLD")
            ...
            for gold_dse_arg in gold_args:
                    ...
                    if gold_arg_label == sys_arg_label:
                        for i in range(gold_arg_start, gold_arg_end + 1):
                            if sys_arg_start <= i <= sys_arg_end:
                                count += 1
                        if count > 0:
                            proportions.append([count, gold_arg_label, sys_arg_end - sys_arg_start + 1])
                max_overlapped = sorted(proportions, reverse=True)[0]
                # print("gold recall", proportions)
                ...
                if max_overlapped_label == "AGENT":
                    # hp_recall_matched += 1.0 * max_overlapped[0] / gold_arg_length
                ...
                elif max_overlapped_label == "TARGET":
                    target_metic.add_proportion_matched(1.0 * max_overlapped[0] / gold_arg_length)
                    matched_proportion_num += 1.0 * max_overlapped[0] / gold_arg_length
                else:
                    assert max_overlapped_label == "O" and max_overlapped[0] == 0
                    holder_metric.add_proportion_matched(max_overlapped[0])
                    target_metic.add_proportion_matched(max_overlapped[0])

            # labeled
            for sys_dse_arg in sys_args:
                sys_arg_start, sys_arg_end, sys_arg_label = sys_dse_arg
                sys_arg_num = sys_arg_end - sys_arg_start + 1
                sys_proportion_num += 1
                proportions = [[0, "O", 0]]
                for gold_dse_arg in gold_args:
                    gold_arg_start, gold_arg_end, gold_arg_label = gold_dse_arg
                    count = 0
                    if sys_arg_label == gold_arg_label:
                        for i in range(sys_arg_start, sys_arg_end + 1):
                            if gold_arg_start <= i <= gold_arg_end:
                                count += 1
                        if count > 0:
                            proportions.append([count, sys_arg_label, gold_arg_end - gold_arg_start + 1])
                            break
                max_overlapped = sorted(proportions, reverse=True)[0]
                ...
                # print(max_overlapped)
                if max_overlapped_label == "AGENT":
                    # not / sys_arg_num, should / gold_arg_num according to the script in
                    # https://github.com/amarasovic/naacl-mpqa-srl4orl/blob/master/eval_utils.py
                    hp_precision_mathced += 1.0 * max_overlapped[0] / sys_arg_num
                    # holder_metric.add_proportion_matched(max_overlapped[0])
                elif max_overlapped_label == "TARGET":
                    pass
                    # target_metic.add_proportion_matched(1.0 * max_overlapped[0] / sys_arg_num)
                    # target_metic.add_proportion_gold(max_overlapped[2])
                else:
                    assert max_overlapped_label == "O" and max_overlapped[0] == 0
                    holder_metric.add_proportion_matched(max_overlapped[0])
                    target_metic.add_proportion_matched(max_overlapped[0])
