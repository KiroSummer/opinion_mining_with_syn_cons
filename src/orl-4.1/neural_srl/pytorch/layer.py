...
class NonLinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init.normal(self.linear.bias, 0.0, 0.01)
...
class SpanClassifier(nn.Module):
    def __init__(self, input_size, inner_size, output_size, span_size, argu_size, drop=0.0, use_boundary=False):
        super(SpanClassifier, self).__init__()
        self.left_boundary, self.right_boundary = None, None
        if use_boundary is True:
            self.left_boundary = MLPRepScorer(input_size, inner_size, output_size, dropout=drop)
            self.right_boundary = MLPRepScorer(input_size, inner_size, output_size, dropout=drop)

        self.span_reps_0 = nn.Linear(input_size, argu_size, bias=False)
        self.span_reps_drop_0 = nn.Dropout(drop)

        # self.span_emb_size = 2 * span_size
        # self.span_reps = nn.Linear(self.span_emb_size, argu_size_u)
        # self.span_reps_drop = nn.Dropout(self.dropout)

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.span_reps_0.weight)
        initializer_1d(self.span_reps_0.bias, nn.init.xavier_uniform_)

    def forward(self, x):
        span_starts_scores = self.left_boundary.forward(x)
        span_ends_scores = self.right_boundary.forward(x)

        span_reps = self.span_reps_drop_0(
            torch.relu(self.span_reps_0.forward(x))
        )
        return span_starts_scores, span_ends_scores, span_reps
...
class MyLSTM1(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, batch_first=False,
        ...
        self.fcells, self.f_dropout = [], []
        self.bcells, self.b_dropout = [], []
        for layer in range(num_layers):
            layer_input_size = input_size if layer == 0 else hidden_size
            self.fcells.append(nn.LSTMCell(input_size=layer_input_size, hidden_size=hidden_size))
            self.f_dropout.append(DropoutLayer(hidden_size, 0.1))
            if self.bidirectional:
                self.bcells.append(nn.LSTMCell(input_size=hidden_size, hidden_size=hidden_size))
                self.b_dropout.append(DropoutLayer(hidden_size, 0.1))
    ...
    def reset_parameters(self):  # modified by kiro
        for name, param in self.named_parameters():
            ...
            if "bias" in name:
                # nn.init.normal(self.__getattr__(name), 0.0, 0.01)
                nn.init.constant(self.__getattr__(name), 0.0)  # different from zhang's 0

    @staticmethod
    def _forward_rnn(cell, input, masks, initial, drop_masks=None):
        max_time = input.size(0)
        output = []
        hx = initial
        for time in range(max_time):
    ...
    @staticmethod
    def _forward_brnn(cell, input, masks, initial, drop_masks=None):
        max_time = input.size(0)
        output = []
        hx = initial
        for time in reversed(range(max_time)):
    ...
    def forward(self, input, masks, initial=None):
        ...
        for layer in range(self.num_layers):
            hidden_mask = None
            if self.training:  # when training, use the dropout
                hidden_mask = self.f_dropout[layer]
            layer_output, (layer_h_n, layer_c_n) = MyLSTM1._forward_rnn(cell=self.fcells[layer],
                                                                        input=input, masks=masks, initial=initial,
                                                                        drop_masks=hidden_mask)
            ...
            if self.bidirectional:
                if self.training:
                    hidden_mask = self.b_dropout[layer]
                blayer_output, (blayer_h_n, blayer_c_n) = MyLSTM1._forward_brnn(cell=self.bcells[layer],
                                                                                input=layer_output, masks=masks,
                                                                                initial=initial, drop_masks=hidden_mask)
