...
from .layer import MyLSTM, NonLinear, Biaffine
...
def drop_input_independent(word_embeddings, tag_embeddings, dropout_emb):
    ...
    word_masks = torch.Tensor(torch.bernoulli(word_masks))
    word_masks.requires_grad = False
    tag_masks = tag_embeddings.data.new(batch_size,
                                        seq_length).fill_(1 - dropout_emb)
    tag_masks = torch.Tensor(torch.bernoulli(tag_masks))
    tag_masks.requires_grad = False
...
def drop_sequence_sharedmask(inputs, dropout, batch_first=True):
    ...
    drop_masks = torch.Tensor(batch_size, hidden_size).fill_(1 - dropout)
    drop_masks = torch.Tensor(torch.bernoulli(drop_masks)).type(inputs.type())
    drop_masks.requires_grad = False
    drop_masks = drop_masks / (1 - dropout)
    drop_masks = torch.unsqueeze(drop_masks,
                                 dim=2).expand(-1, -1,
                                               seq_length).permute(2, 0, 1)
    inputs = inputs * drop_masks

    return inputs.transpose(1, 0)


class ParserModel(nn.Module):  # build a biaffine parser model
    def __init__(self, vocab, config, pretrained_embedding):
        ...
        self.extword_embed = nn.Embedding(vocab.extvocab_size,
                                          config.word_dims,
                                          padding_idx=0)
        self.tag_embed = nn.Embedding(vocab.tag_size,
                                      config.tag_dims,
                                      padding_idx=0)
        ...
        self.arc_num = int(config.mlp_arc_size / 100)  # config: 500
        self.rel_num = int(config.mlp_rel_size / 100)  # config: 100
    ...
    def forward(
            self, words, extwords, tags,
            masks):  # words [batch, max_sentence_length], padding with zeros
        ...
        if self.training:
            x_embed, x_tag_embed = drop_input_independent(
                x_embed, x_tag_embed, self.config.dropout_emb)
        ...
        if self.training:
            outputs = drop_sequence_sharedmask(outputs,
