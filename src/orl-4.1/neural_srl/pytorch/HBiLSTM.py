...
class HBiLSTM(nn.Module):
    def __init__(self, args):
        ...
        self.gate_layer = self.init_Linear(in_fea=in_feas,
                                           out_fea=in_feas,
                                           bias=True)

        # if bidirection convert dim
        self.convert_layer = self.init_Linear(
            in_fea=self.args.lstm_hidden_dim * 2,
            out_fea=self.args.embed_dim,
            bias=True)
    ...
    def init_hidden(self, num_layers, batch_size):
        ...
        if self.args.cuda is True:
            return (Variable(
                torch.zeros(2 * num_layers, batch_size,
            ...
            return (Variable(
                torch.zeros(2 * num_layers, batch_size, self.hidden_dim)),
                Variable(
                    torch.zeros(2 * num_layers, batch_size, self.hidden_dim))
                )
    ...
    def forward(self, x, hidden):
        ...
        self.gate_layer = self.init_Linear(in_fea=in_fea,
                                           out_fea=out_fea,
                                           bias=True)
        # self.gate_layer = self.init_Linear(in_fea=out_fea,
        # out_fea=in_fea, bias=True)
        # the first way to convert 3D tensor to the Linear
        source_x = source_x.contiguous()
        information_source = source_x.view(
            source_x.size(0) * source_x.size(1), source_x.size(2))
        information_source = self.gate_layer(information_source)
        information_source = information_source.view(
            source_x.size(0), source_x.size(1), information_source.size(1))
        ...
        # the information_source compare to the source_x is for the same
        # size of x,y,H,T
        ...
        information_convert = information_flow.view(
            information_flow.size(0) * information_flow.size(1),
            information_flow.size(2))
        information_convert = self.convert_layer(information_convert)
        information_convert = information_convert.view(
            information_flow.size(0), information_flow.size(1),
...
class HBiLSTM_model(nn.Module):
    def __init__(self, args):
        ...
        self.highway = nn.ModuleList(
            [HBiLSTM(args) for _ in range(args.layer_num_highway)])
        self.output_layer = self.init_Linear(in_fea=self.args.embed_dim,
                                             out_fea=self.C,
                                             bias=True)
